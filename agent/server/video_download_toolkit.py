...
class VideoDownloaderToolkit(BaseToolkit):
    ...
    def __init__(
        ...
        # setupdefaultdownloaddirectory
        default_download_dir = "./workspaces/test_video/workspace"
        
        # English: 只有当没有指定downloaddirectory且使用临时directory时才进行清理
        self._cleanup = download_directory is None and default_download_dir.startswith(tempfile.gettempdir())
        self._cookies_path = cookies_path

        self._download_directory = Path(
            download_directory or default_download_dir
        ).resolve()
